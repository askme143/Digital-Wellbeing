...
import com.yeongil.focusaid.databinding.FragmentTriggerBinding
import com.yeongil.focusaid.utils.navigateSafe
import com.yeongil.focusaid.viewModel.viewModel.rule.RuleEditViewModel
import com.yeongil.focusaid.viewModel.item.ACTIVITY_TRIGGER_TITLE
import com.yeongil.focusaid.viewModel.item.LOCATION_TRIGGER_TITLE
import com.yeongil.focusaid.viewModel.item.TIME_TRIGGER_TITLE
...
class TriggerFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        ruleEditViewModel.itemEditEvent.observe(viewLifecycleOwner) { event ->
            event.getContentIfNotHandled()?.let {
                Toast.makeText(context, "조건이 수정되었습니다.", Toast.LENGTH_SHORT).show()
            }
        }
